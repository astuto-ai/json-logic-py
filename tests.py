...
class JSONLogicTest(unittest.TestCase):
    ...
    def test_if(self):
        ...
        self.assertEqual(
            jsonLogic(
                {"if": [
                    {"lt": [{"var": "temp"}, 0]}, "freezing",
                    {"lt": [{"var": "temp"}, 100]}, "liquid",
                    "gas"
                ]},
                {"temp": 200}
            ),
    ...
    def test_cmp(self):
        ...
        # Greater than:
        self.assertTrue(jsonLogic({"gt": [2, 1]}))
        # Greater than or equal to:
        self.assertTrue(jsonLogic({"gte": [1, 1]}))
        # Less than:
        self.assertTrue(jsonLogic({"lt": [1, 2]}))
        # Less than or equal to:
        self.assertTrue(jsonLogic({"lte": [1, 1]}))

    def test_between(self):
        """
        You can use a special case of lt and lte to test that one value
        is between two others.
        """
        # Between exclusive:
        self.assertTrue(jsonLogic({"lt": [1, 2, 3]}))
        self.assertFalse(jsonLogic({"lt": [1, 1, 3]}))
        self.assertFalse(jsonLogic({"lt": [1, 4, 3]}))
        # Between inclusive:
        self.assertTrue(jsonLogic({"lte": [1, 2, 3]}))
        self.assertTrue(jsonLogic({"lte": [1, 1, 3]}))
        self.assertFalse(jsonLogic({"lte": [1, 4, 3]}))
